...
router.get('/name/:name', (req, res) => {
    item.findOne({ name: req.params.name })
        ...
        .catch(err => res.status(400).json({ error: 'Error fetching item by name', details: err }));
});
...
router.get('/type/:type', (req, res) => {
    item.find({ type: req.params.type })
        ...
        .catch(err => res.status(400).json({ error: 'Error fetching items by type', details: err }));
});
...
router.post('/', (req, res) => {
    if (Array.isArray(req.body)) {
        item.insertMany(req.body)
            ...
            .catch(err => res.status(400).json({ error: 'Unable to add these items', details: err }));
    } else {
        item.create(req.body)
            .then(item => res.json({ msg: 'Item added successfully', item }))
            .catch(err => res.status(400).json({ error: 'Unable to add this item', details: err }));
    }
});

router.put('/:id', (req, res) => {
    item.findByIdAndUpdate(req.params.id, req.body, { new: true }) // Add { new: true } to return the updated document
        .then(item => res.json({ msg: 'Updated successfully', item }))
        .catch(err => res.status(400).json({ error: 'Unable to update the Database', details: err }));
});

router.delete('/:id', (req, res) => {
    item.findByIdAndDelete(req.params.id)
        .then(item => res.json({ msg: 'Item entry deleted successfully', item }))
        .catch(err => res.status(404).json({ error: 'No such item', details: err }));
});
